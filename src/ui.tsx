...
function App() {
  ...
  const [lang, setLang] = React.useState<string>("typescript");
  ...
  const languageList = ["TypeScript", "JavaScript", "Rescript"];

  const selectTag = languageList.map((val) => {
    return (
      <option
  ...
  return (
    <div>
      <p className={styles.title}>React Native Code Generator</p>
      <br></br>
      <div>
        <button
          ...
          onClick={() => {
            ...
            setText([]);
            parent.postMessage(
              { pluginMessage: { type: "generate-code" , lang: lang} },
              "*"
            );
          }}
        >
      ...
      <h4 hidden={text.length <= 0}>
        ...
        <br></br>
        In this case you want to replace rating with a state variable we can
        update the text in the textField as
        <br></br>
...
function updateTextNodes(nodes: OutputTree, textNode: Array<TextNodeProps>) {
  ...
  if (node) {
    node.childrens = [
      "{ React.string(" +
        JSON.stringify(textNode[0].childrens)
          .replace(RegExp("<v>", "g"), '" + ')
          .replace(RegExp("</v>", "g"), '+ "') +
        ")}",
    ];
